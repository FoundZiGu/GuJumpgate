  ...
  function createFreemailProvider(deps = {}) {
    ...
    async function fetchFreemailAddress(state, options = {}) {
      ...
      if (activeDomain && domainIndex < 0) {
        throw new Error(
          `freemail 主域名 ${activeDomain} 不在 /api/domains 返回列表中。`
        );
      }
    ...
    function resolveFreemailPollTargetEmail(
      state = {},
      pollPayload = {},
      config = getFreemailConfig(state)
    ) {
    ...
    async function pollFreemailVerificationCode(step, state, pollPayload = {}) {
      const latestState = state || await getState();
      const config = ensureFreemailConfig(latestState, { requireAuth: true });
      const targetEmail = resolveFreemailPollTargetEmail(latestState, pollPayload, config);
      if (!targetEmail) {
        throw new Error([
          'freemail 轮询前缺少目标邮箱地址，',
          '请先生成或填写注册邮箱。',
        ].join(''));
      }
      ...
      throw lastError || new Error(
        `步骤 ${step}：未在 freemail 中找到新的匹配验证码。`
      );
    }
